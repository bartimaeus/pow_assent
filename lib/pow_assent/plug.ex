defmodule PowAssent.Plug do
  ...
  defp maybe_authenticate(
    ...
    case Plug.current_user(conn) do
      nil ->
        conn
        |> authenticate(user_params, user_identity_params)
        |> case do
          {:ok, conn} -> conn
          {:error, conn} -> conn
        end
  ...
  defp maybe_create_user(conn) do
    conn
    |> Plug.current_user()
    |> maybe_create_user(conn)
  end

  defp maybe_create_user(nil, %{private: %{pow_assent_registration: false}} = conn) do
    conn
  ...
  """
  @spec authenticate(Conn.t(), map(), map()) :: {:ok, Conn.t()} | {:error, Conn.t()}
  def authenticate(conn, %{"email" => email}, %{"provider" => provider, "uid" => uid}) do
    ...
    |> case do
      nil ->
        Operations.get_user_by_email(email, config)
        |> case do
          nil -> {:error, conn}
      ...
      user ->
        {:ok, create_session(conn, user, provider, config)}
    end
  end
